...
## 5. 閉環自適應學習演算法 (Adaptive Learning)

系統利用近 60 天實戰表現進行動態回饋調整，避免單一策略在特定盤勢（如盤整盤、急跌波）持續失血：

### 5.1 評估指標
- **勝率 (Win Rate)**：獲利次數 / 總結算次數。
- **盈虧比 (Profit Factor)**：`總獲利金額比例 / 總虧損金額比例`。

### 5.2 動態權重調整公式
基準權重為 `1.0`，依實戰指標動態調整並限制在 `[0.2, 2.5]` 區間：
```
勝率權重加成 = (WinRate - 0.50) * 2.5
...
| 狀態 | 判定條件 | 系統行為 |
| :--- | :--- | :--- |
| **ScaledUp (自動加權)** | 勝率 ≥ 60% 且 盈虧比 ≥ 1.3 | 權重上調（最高 2.5x），擴大推薦優先權 |
| **Active (標準運行)** | 40% ≤ 勝率 < 60% | 維持基準權重（0.8x ~ 1.2x）正常運作 |
| **Demoted (降權保護)** | 勝率 < 40% 或 盈虧比 < 0.8 | 權重調降（最低 0.2x），減少該策略推薦排名 |
| **Hibernating (休眠保護)** | 勝率 < 30%（連續嚴重失準） | 降低至底限，系統發出警示並暫停主力推薦 |
